...
2. Determine whether the prompt refers to an issue or a PR. If it's a PR, extract the issue number from the branch name (e.g., branch `topic/2026.0/1234-fix-something` → issue `#1234`). Use `gh pr view` to get the branch name if needed. From this point on, work with the resolved issue number.
3. Read the issue on GitHub including ALL comments.
4. Check for existing topic branches and PRs. Branch names follow the pattern `topic/{version}/{issue_number}-*` (e.g. `topic/2026.0/1234-fix-something`). Use the GitHub API to check all repos in parallel:
   ```bash
